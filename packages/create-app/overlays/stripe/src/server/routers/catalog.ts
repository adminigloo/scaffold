 ...
 *
 * MERGED NOW, in `src/server/audit.ts`, which is generated because this router
 * only exists in a project that sells something. It spreads this fragment
 * alongside the admin and invitation ones through `contributedBy`, so a
 * duplicate key throws at boot instead of letting the last spread decide the
 * label and the `sensitive` flag. That file is also where `admin.recentAudit`
 * reads its labels from — before it existed there were two registries, neither
 * could see a collision in the other, and a catalog action rendered in the
 * audit viewer as its raw key.
 *
 * Writes still go through the local registry below. Importing the composed one
 * here would make `src/server/audit.ts` and this module a cycle, and the two
 * agree by construction: the composed registry is built FROM this fragment.
 */
