 ...
 * sign-up is a multi-step flow — verify the address, add a second factor,
 * discover the account already exists — and each step is a separate navigation
 * that would lose a value this page was holding. `signInUrl` carries it too, so
 * the "already have an account" link inside Clerk's own card does not drop it.
 *
  ...
  return (
    <main className="grid min-h-[70vh] place-items-center px-6 py-12">
      <SignUp
        ...
        signInUrl={
          back === null ? undefined : `/sign-in?redirect_url=${encodeURIComponent(back)}`
        }
      />
    </main>
  );
