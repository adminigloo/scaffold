 ...
 *
 * Both casings are listed. Node lowercases inbound header names, but a
 * hand-built object literal, an `Object.fromEntries(request.headers)` dump and
 * anything copied out of a provider's docs do not — and fast-redact matches
 * path segments byte for byte, so `headers.Authorization` walks straight past
 * a list that only knows `headers.authorization`.
 *
