 ...
 *
 * Returns bigint. For one request a double would hold the intermediate, but
 * this is also the function you re-price a month of usage with, and 4 billion
 * tokens against a $3.00/MTok rate multiplies out to 1.2e16 — past the point
 * where a double still represents every integer. The column it lands in is
 * bigint either way, so returning a Number would only add a lossy hop.
 */
